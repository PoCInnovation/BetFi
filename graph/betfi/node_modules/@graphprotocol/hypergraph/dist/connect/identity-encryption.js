...
export const encryptIdentity = async (signer, keys) => {
...
export const decryptIdentity = async (signer, ciphertext, nonce) => {
...
export const encryptAppIdentity = async (signer, appIdentityAddress, appIdentityAddressPrivateKey, permissionId, keys) => {
  const nonce = randomBytes(32);
  const message = signatureMessage(nonce);
  const signature = await signer.signMessage(message);
  // Check that the signature is valid
...
export const decryptAppIdentity = async (signer, ciphertext, nonce) => {
  const message = signatureMessage(hexToBytes(nonce));
  const signature = await signer.signMessage(message);
  // Check that the signature is valid
  const valid = await verifyMessage({
