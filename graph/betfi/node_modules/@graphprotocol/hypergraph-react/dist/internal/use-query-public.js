...
`;
const convertPropertyValue = (property, key, type) => {
...
const convertRelations = (queryEntity, type, mappingEntry, mapping) => {
  const rawEntity = {};
  for (const [key, relationId] of Object.entries(mappingEntry?.relations ?? {})) {
    ...
    const relationTypeName = annotations[Object.getOwnPropertySymbols(annotations).find(sym => sym.description === 'effect/annotation/Identifier')];
...
export const parseResult = (queryData, type, mappingEntry, mapping) => {
...
export const useQueryPublic = (type, params) => {
  const {
    enabled = true,
    include,
  ...
  const relationTypeIdsLevel1 = [];
  const relationTypeIdsLevel2 = [];
  ...
  const result = useQueryTanstack({
    queryKey: ['hypergraph-public-entities', typeName, space, mappingEntry?.typeIds, relationTypeIdsLevel1, relationTypeIdsLevel2
    ...
    queryFn: async () => {
      ...
      const result = await request(GEO_API_TESTNET_ENDPOINT, queryDocument, {
        spaceId: space,
...
//# sourceMappingURL=use-query-public.js.map