...
export function useOwnSpaceInbox({
  ...
  useEffect(() => {
      ...
      try {
        if (!ownInboxId) {
          if (inboxId) {
            ...
            const inboxId = await ensureSpaceInbox({
              spaceId,
              isPublic,
              authPolicy
            });
    ...
    const fetchInbox = async () => {
      try {
        if (ownInboxId) {
          await getLatestSpaceInboxMessages({
            spaceId,
            inboxId: ownInboxId
          });
        }
  ...
  }, [spaceId, inboxId, ownInboxId, getLatestSpaceInboxMessages, ensureSpaceInbox, autoCreate, isPublic, authPolicy, space?.inboxes[0]?.inboxId]);
...
//# sourceMappingURL=useOwnSpaceInbox.js.map