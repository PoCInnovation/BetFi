...
import { Connect, Identity, Inboxes, Key, Messages, SpaceEvents, Utils, store } from '@graphprotocol/hypergraph';
import { QueryClient, QueryClientProvider } from '@tanstack/react-query';
import { useSelector as useSelectorStore } from '@xstate/store/react';
import { Effect, Exit } from 'effect';
import * as Schema from 'effect/Schema';
import { createContext, useCallback, useContext, useEffect, useLayoutEffect, useRef, useState } from 'react';
const decodeResponseMessage = /*#__PURE__*/Schema.decodeUnknownEither(Messages.ResponseMessage);
const queryClient = /*#__PURE__*/new QueryClient();
const CHAIN = Connect.GEO_TESTNET;
const RPC_URL = Connect.TESTNET_RPC_URL;
export const HypergraphAppContext = /*#__PURE__*/createContext({
  logout() {
...
export function HypergraphAppProvider({
  ...
  useEffect(() => {
    ...
    const applyUpdates = async (spaceId, spaceSecretKey, automergeDocHandle, updates) => {
      const verifiedUpdates = await Promise.all(updates.updates.map(async update => {
        ...
        const authorIdentity = await Identity.getVerifiedIdentity(update.accountAddress, syncServerUri, CHAIN, RPC_URL);
    ...
    const getVerifiedIdentity = accountAddress => {
      ...
      if (message._tag === 'Right') {
        ...
        switch (response.type) {
          ...
          case 'space':
            {
              ...
              for (const event of response.events) {
                ...
                const applyEventResult = await Effect.runPromiseExit(SpaceEvents.applyEvent({
                  state,
                  event,
                  getVerifiedIdentity
                }));
              ...
              const inboxes = response.inboxes.map(inbox => {
                return {
                  ...
                  secretKey: Utils.bytesToHex(Messages.decryptMessage({
          ...
          case 'space-inbox-message':
            {
              const inbox = store.getSnapshot().context.spaces.find(s => s.id === response.spaceId)?.inboxes.find(i => i.inboxId === response.inboxId);
              if (!inbox) {
                console.error('Inbox not found', response.inboxId);
                return;
              }
              const isValid = await Inboxes.validateSpaceInboxMessage(response.message, inbox, response.spaceId, syncServerUri, CHAIN, RPC_URL);
              if (!isValid) {
          ...
          case 'account-inbox-message':
            {
              ...
              const isValid = await Inboxes.validateAccountInboxMessage(response.message, inbox, identity.address, syncServerUri, CHAIN, RPC_URL);
              if (!isValid) {
                console.error('Invalid message', response.message, inbox.inboxId);
                return;
              }
          ...
          case 'account-inbox-messages':
            {
              ...
              const validSignatures = await Promise.all(response.messages.map(
              // If the message has a signature, check that the signature is valid for the authorAccountAddress
              async message => {
                return Inboxes.validateAccountInboxMessage(message, inbox, identity.address, syncServerUri, CHAIN, RPC_URL);
              }));
              let lastMessageClock = new Date(0);
              const messages = response.messages.filter((message, index) => validSignatures[index]).map(message => {
                try {
                  const decryptedMessage = Inboxes.decryptInboxMessage({
  ...
  const createSpaceForContext = useCallback(async ({
    ...
    const spaceEvent = await Effect.runPromise(SpaceEvents.createSpace({
      author: {
        accountAddress: identity.address,
        encryptionPublicKey,
        signaturePrivateKey,
  ...
  const getSmartSessionClientForContext = useCallback(async () => {
    ...
    return Connect.getSmartSessionClient({
      ...
      sessionPrivateKey: identity.addressPrivateKey,
      chain: chainId === Connect.GEOGENESIS.id ? Connect.GEOGENESIS : Connect.GEO_TESTNET,
      rpcUrl: chainId === Connect.GEOGENESIS.id ? Connect.GEOGENESIS.rpcUrls.default.http[0] : Connect.GEO_TESTNET.rpcUrls.default.http[0]
    });
  }, [identity, chainId]);
  const createSpaceInboxForContext = useCallback(async ({
    space,
    isPublic,
    authPolicy
  }) => {
  ...
  const getLatestSpaceInboxMessagesForContext = useCallback(async ({
    spaceId,
    inboxId
  }) => {
    const storeState = store.getSnapshot();
  ...
  const listPublicSpaceInboxesForContext = useCallback(async ({
    spaceId
  }) => {
    return await Inboxes.listPublicSpaceInboxes({
      spaceId,
      syncServerUri
    });
  }, [syncServerUri]);
  const getSpaceInboxForContext = useCallback(async ({
    spaceId,
    inboxId
  }) => {
    return await Inboxes.getSpaceInbox({
      spaceId,
      inboxId,
      syncServerUri
    });
  }, [syncServerUri]);
  const sendSpaceInboxMessageForContext = useCallback(async ({
    spaceId,
    inboxId,
    message,
    encryptionPublicKey,
  ...
  const createAccountInboxForContext = useCallback(async ({
    isPublic,
    authPolicy
  }) => {
    if (!identity) {
  ...
  const getLatestAccountInboxMessagesForContext = useCallback(async ({
    accountAddress,
    inboxId
  }) => {
    const storeState = store.getSnapshot();
  ...
  const listPublicAccountInboxesForContext = useCallback(async ({
    accountAddress
  }) => {
    return await Inboxes.listPublicAccountInboxes({
      accountAddress,
      syncServerUri
    });
  }, [syncServerUri]);
  const getAccountInboxForContext = useCallback(async ({
    accountAddress,
    inboxId
  }) => {
    return await Inboxes.getAccountInbox({
      accountAddress,
      inboxId,
      syncServerUri
    });
  }, [syncServerUri]);
  const sendAccountInboxMessageForContext = useCallback(async ({
    message,
    accountAddress,
    inboxId,
    encryptionPublicKey,
  ...
  const acceptInvitationForContext = useCallback(async ({
    invitation
  }) => {
    if (!identity) {
      throw new Error('No identity   found');
    ...
    const spaceEvent = await Effect.runPromiseExit(SpaceEvents.acceptInvitation({
      author: {
        accountAddress: identity.address,
        signaturePublicKey,
        encryptionPublicKey,
  ...
  const subscribeToSpace = useCallback(params => {
    const message = {
  ...
  const inviteToSpace = useCallback(async ({
    ...
    const spaceEvent = await Effect.runPromiseExit(SpaceEvents.createInvitation({
      author: {
        accountAddress: identity.address,
        signaturePublicKey,
        encryptionPublicKey,
  ...
  const getVerifiedIdentity = useCallback(accountAddress => {
    return Identity.getVerifiedIdentity(accountAddress, syncServerUri, CHAIN, RPC_URL);
  }, [syncServerUri]);
  const ensureSpaceInboxForContext = useCallback(async ({
    spaceId,
    isPublic = true,
  ...
  const redirectToConnectForContext = useCallback(params => {
    const {
      storage,
      successUrl,
      redirectFn,
  ...
  const processConnectAuthSuccessForContext = useCallback(params => {
    const {
      storage,
      ciphertext,
      nonce
    ...
    try {
      const parsedAuthParams = Effect.runSync(Connect.parseCallbackParams({
        ciphertext,
        nonce,
        storedNonce,
        storedExpiry,
  ...
  return _jsx(HypergraphAppContext.Provider, {
    value: {
      logout,
      setIdentity,
      invitations,
    ...
    children: _jsx(QueryClientProvider, {
      client: queryClient,
      children: _jsx(RepoContext.Provider, {
        value: repo,
        children: children
      })
    })
...
//# sourceMappingURL=HypergraphAppContext.js.map