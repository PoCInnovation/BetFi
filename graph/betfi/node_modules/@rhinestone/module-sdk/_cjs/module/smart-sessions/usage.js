...
const getSessionNonce = async ({ client, account, permissionId, }) => {
    return (await client.readContract({
        ...
        args: [permissionId, (0, account_1.isAccount)(account) ? account.address : account],
    }));
};
...
const getVerifySignatureResult = async ({ client, account, sender, hash, signature, }) => {
    ...
    const { data } = await client.call({
        ...
        to: constants_1.GLOBAL_CONSTANTS.SMART_SESSIONS_ADDRESS,
        data: calldata,
    });
    return (data ===
...
const encodeUseOrEnableSmartSessionSignature = async ({ account, client, permissionId, signature, enableSessionData, }) => {
    ...
    return sessionEnabled
        ? (0, viem_1.encodePacked)(['bytes1', 'bytes32', 'bytes'], [types_1.SmartSessionMode.USE, permissionId, signature])
...
const getEnableSessionsAction = ({ sessions, }) => {
    ...
    return {
        to: constants_1.GLOBAL_CONSTANTS.SMART_SESSIONS_ADDRESS,
        target: constants_1.GLOBAL_CONSTANTS.SMART_SESSIONS_ADDRESS,
        value: BigInt(0),
        callData: data,
        data,
    };
...
const getEnableSessionDetails = async ({ sessions, sessionIndex, enableMode, account, clients, enableValidatorAddress, permitGenericPolicy = false, permitAdminAccess = false, ignoreSecurityAttestations = false, sessionNonces, }) => {
    const chainDigests = [];
    const chainSessions = [];
    for (const session of sessions) {
        ...
        const sessionNonce = sessionNonces?.[currentIndex] ??
            (await (0, exports.getSessionNonce)({
                client,
                account,
                permissionId,
            }));
        ...
        chainSessions.push({
            ...
            session: {
                ...
                account: account.address,
                smartSession: constants_1.GLOBAL_CONSTANTS.SMART_SESSIONS_ADDRESS,
                nonce: sessionNonce,
            },
        });
    }
